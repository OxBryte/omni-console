...
export default function PlaygroundView() {
  ...
  // Storage Sandbox Tab
  const [sandboxStorageTab, setSandboxStorageTab] = useState<
    "cookies" | "local" | "session"
  >("local");
  const [sandboxCookies, setSandboxCookies] = useState<
    { key: string; value: string }[]
  >([]);
  const [sandboxLocal, setSandboxLocal] = useState<
    { key: string; value: string }[]
  >([]);
  ...
  const [workbenchEvents, setWorkbenchEvents] = useState<WorkbenchEvent[]>([
    {
      type: "SYSTEM",
      msg: "Developer Lab Simulator Initialized.",
      time: "13:08:00",
      status: "active",
    },
    {
      type: "CONSOLE",
      msg: "Core interceptor bound to window console.",
      time: "13:08:01",
      status: "success",
    },
  ]);
  ...
  useEffect(() => {
    ...
    const metricsInterval = setInterval(() => {
      ...
      setFpsMetric((prev) => {
        if (prev < 59)
          return Math.min(60, prev + Math.floor(Math.random() * 2) + 1);
        return 60;
      });
    }, 1000);
  ...
  const runSelectedScript = () => {
    ...
    if (selectedScript === "auth") {
      addWorkbenchEvent(
        "SYSTEM",
        "Running Authenticate Flow script...",
        "active",
      );
      setCpuMetric(14.8);
      ...
      setTimeout(() => {
        ...
        addWorkbenchEvent(
          "CONSOLE",
          "info: AuthManager: Requesting token exchange...",
          "active",
        );
      ...
      setTimeout(() => {
        ...
        fetch("https://jsonplaceholder.typicode.com/posts/1")
          ...
          .then(() => {
            console.log(
              "AuthManager: Token exchanged successfully. Session response ok.",
              {
                token_type: "Bearer",
                expires_in: 3600,
                user_id: 89402,
            ...
            addWorkbenchEvent(
              "NETWORK",
              "POST /v1/auth/token - 200 OK (32ms)",
              "success",
            );
            addWorkbenchEvent(
              "CONSOLE",
              "log: AuthManager: Token exchanged successfully.",
              "success",
            );
            setTotalLogCount((prev) => prev + 1);
      ...
      setTimeout(() => {
        ...
        document.cookie = "session_user=apple_developer_tier; path=/";
        console.log(
          "AuthManager: Credentials cached. Storage cookie 'session_user' set.",
        );
        addWorkbenchEvent(
          "STORAGE",
          'Cookie "session_user" updated -> "apple_developer_tier"',
          "success",
        );
      ...
      setTimeout(() => {
        ...
        addWorkbenchEvent(
          "SYSTEM",
          "Authenticate Flow script execution complete.",
          "success",
        );
      }, 2400);
      ...
      addWorkbenchEvent(
        "SYSTEM",
        "Running E-Commerce Checkout script...",
        "active",
      );
      setCpuMetric(12.2);
      ...
      setTimeout(() => {
        setScriptProgress(25);
        console.log(
          "CheckoutService: Starting processing check for studio-display...",
        );
        addWorkbenchEvent(
          "CONSOLE",
          "log: CheckoutService: Starting processing check...",
          "active",
        );
        setTotalLogCount((prev) => prev + 1);
      }, 600);
      ...
      setTimeout(() => {
        ...
        fetch("https://jsonplaceholder.typicode.com/posts/1")
          ...
          .then(() => {
            console.warn(
              "CheckoutService: Legacy payment gateway API endpoint is deprecated.",
            );
            addWorkbenchEvent(
              "NETWORK",
              "GET /v1/checkout/verify - 200 OK (38ms)",
              "success",
            );
            addWorkbenchEvent(
              "CONSOLE",
              "warn: Legacy checkout endpoint is deprecated.",
              "warning",
            );
            setTotalLogCount((prev) => prev + 1);
            setNetworkRequestCount((prev) => prev + 1);
      ...
      setTimeout(() => {
        ...
        console.error(
          "CheckoutService: Checkout transaction failed.",
          new Error("PaymentGatewayTimeout"),
        );
        addWorkbenchEvent(
          "CONSOLE",
        ...
        addWorkbenchEvent(
          "ERROR",
          "PaymentGatewayException: Processing timeout",
          "error",
        );
        setTotalLogCount((prev) => prev + 1);
        ...
        setTimeout(() => {
          try {
            throw new Error(
              "PaymentGatewayException: Processing connection refused by gateway provider.",
            );
          } catch (e: any) {
            window.dispatchEvent(
              new ErrorEvent("error", { error: e, message: e.message }),
            );
          }
        }, 50);
      }, 1800);
      ...
      setTimeout(() => {
        ...
        addWorkbenchEvent(
          "SYSTEM",
          "Checkout script finished with fatal runtime warning.",
          "error",
        );
      }, 2400);
      ...
      addWorkbenchEvent(
        "SYSTEM",
        `Launching Virtualization Benchmark: logging ${stressLogCount} entries...`,
        "active",
      );
      setCpuMetric(88.4);
      setFpsMetric(26);
      ...
      setTimeout(() => {
        ...
        console.log(
          `Benchmark: Initiating rendering stress test of ${stressLogCount} rows...`,
        );
        for (let i = 1; i <= stressLogCount; i++) {
          console.log(
            `Virtualized log row #${i} - OmniConsole frame stress validation test.`,
          );
        }
        console.info(
          `Benchmark: Stress test loop complete. Emitted ${stressLogCount} logs.`,
        );

        addWorkbenchEvent(
          "CONSOLE",
          `log: Emitted ${stressLogCount} virtualized logs.`,
          "success",
        );
      ...
      setTimeout(() => {
        ...
        addWorkbenchEvent(
          "SYSTEM",
          `Virtualization Stress Test complete. List virtualization verified.`,
          "success",
        );
      }, 1800);
    }
  ...
  const sendCustomRequest = async () => {
    ...
    addWorkbenchEvent(
      "SYSTEM",
      `Sending ${apiMethod} request to ${apiPath}...`,
      "active",
    );
    ...
    setTimeout(async () => {
      ...
      if (apiStatus === 404) {
        ...
        url =
          "https://jsonplaceholder.typicode.com/invalid-route-forces-network-500-error";
      } else if (apiMethod === "GET") {
        url = "https://jsonplaceholder.typicode.com/posts/1";
      }

      console.log(
        `API Requester: Dispatching fetch request: ${apiMethod} ${apiPath}`,
      );
      ...
      try {
        ...
        addWorkbenchEvent(
          "NETWORK",
          `${apiMethod} ${apiPath} - ${apiStatus} (Simulated ${apiLatency}ms)`,
          apiStatus >= 400 ? "error" : "success",
        );
        setNetworkRequestCount((prev) => prev + 1);

        if (apiStatus >= 400) {
          console.error(
            `API Requester: Request failed with status code ${apiStatus}. Path: ${apiPath}`,
          );
        } else {
          console.log(
            `API Requester: Request completed with status code ${apiStatus}. Response:`,
            {
              path: apiPath,
              method: apiMethod,
              status: apiStatus,
        ...
        addWorkbenchEvent(
          "NETWORK",
          `${apiMethod} ${apiPath} - Failed (Network Error)`,
          "error",
        );
  ...
  const handleSandboxClearAll = () => {
    if (sandboxStorageTab === "local") {
      ...
      addWorkbenchEvent(
        "STORAGE",
        `sessionStorage: Cleared sandbox`,
        "warning",
      );
    } else if (sandboxStorageTab === "cookies") {
      document.cookie.split(";").forEach((c) => {
  ...
  const triggerUncaughtException = () => {
    setTimeout(() => {
      try {
        throw new Error(
          'Runtime NullPointerException: Cannot read property "id" of null',
        );
      } catch (e: any) {
        window.dispatchEvent(
          new ErrorEvent("error", { error: e, message: e.message }),
  ...
  const triggerUnhandledRejection = () => {
    Promise.reject(
      new Error("Promise Rejected: API Key is expired or invalid."),
    );
  };
    ...
    <div className="playground-container">
      ...
      <div className="playground-hero" onMouseMove={handleHeroMouseMove}>
        ...
        <div className="playground-hero-content">
          ...
          <h1 className="playground-hero-title">
            Developer <span>Lab</span>
          </h1>
          <p className="playground-hero-subtitle">
            Simulate. Instrument. Profile. A high-fidelity sandbox environment
            for testing telemetry captures, cookie keychains, and high-speed
      ...
      <section
        ...
        className="playground-bento-section mx-auto max-w-[1060px]"
      >
        <div
        ...
        >
          ...
          >
            How the OmniConsole intercept system hooks into standard browser
            layers.
          </p>
        </div>
        <div className="playground-bento-grid">
          ...
          <div className="playground-bento-card bento-1x1">
            ...
            <p className="bento-card-desc">
              Intercepts global console logs, wrapping JSON payloads in colored
              syntax trees.
            </p>
            <div className="bento-illustration console-illustration">
              <div className="bento-code-snippet">
                <span className="code-keyword">const</span> raw = console.log;
                <br />
                console.log = (...args) =&gt; &#123;
                <br />
                &nbsp;&nbsp;syncToViewport(args);
          ...
          <div className="playground-bento-card bento-2x1">
            ...
            <p className="bento-card-desc">
              Hooks standard Fetch and XHR protocols to collect request status,
              headers, and latency metrics.
            </p>
            <div
              className="bento-illustration network-illustration"
              style={{
                display: "flex",
          ...
          <div className="playground-bento-card bento-1x2">
            ...
            <p className="bento-card-desc">
              Monitors LocalStorage, SessionStorage, and Document Cookies
              dynamically with reactive delete hooks.
            </p>
            <div
              className="bento-illustration storage-illustration"
              style={{
                flex: 1,
                display: "flex",
                flexDirection: "column",
          ...
          <div className="playground-bento-card bento-1x1">
            ...
            <p className="bento-card-desc">
              Uses virtual scroll recyclers to render 10,000+ logs at 60 FPS
              under 30MB RAM overhead.
            </p>
            <div
              className="bento-illustration virtualization-illustration"
              style={{
                display: "flex",
                gap: "8px",
                alignItems: "center",
          ...
          <div className="playground-bento-card bento-1x1">
            ...
            <p className="bento-card-desc">
              Automatically catches window errors and unhandled rejections with
              exact trace sources.
            </p>
            <div
              className="bento-illustration error-illustration"
              style={{
                display: "flex",
                justifyContent: "center",
                alignItems: "center",
      ...
      <section
        ...
        className="playground-workbench-section mx-auto max-w-[1060px]"
      >
        <div
          className="section-title-wrapper"
          style={{ textAlign: "center", marginBottom: "32px" }}
        >
          ...
          <p
            ...
            style={{ color: "#86868b", fontSize: "14px" }}
          >
            Real-time simulation engine. Mutate state, trigger calls, and
            inspect diagnostics.
          </p>
        </div>
        ...
        <div className="workbench-container">
          ...
          <div className="workbench-build-bar">
            ...
            <div className="build-info-chunk">
              ...
              <span className="value">
                Logs:{" "}
                <span className="counter-val highlight">{totalLogCount}</span>{" "}
                &bull; Network:{" "}
                <span className="counter-val highlight">
                  {networkRequestCount}
                </span>{" "}
                &bull; Storage:{" "}
                <span className="counter-val highlight">{storageCount}</span>
              </span>
            </div>
          </div>
          ...
          <div className="workbench-layout">
            ...
            <div className="workbench-controls-column">
              ...
              <div className="workbench-card">
                ...
                <p className="card-desc">
                  Select and execute a prebuilt javascript application flow in
                ...
                >
                  ...
                  >
                    <option value="auth">
                      User Authentication Flow (auth.ts)
                    </option>
                    <option value="checkout">
                      E-Commerce Checkout (checkout.ts)
                    </option>
                    <option value="benchmark">
                      Virtual List Performance Benchmark (stressTest.ts)
                    </option>
                  </select>
                ...
                {scriptRunning && (
                  <div
                    className="script-progress-bar-wrap"
                    style={{
                      height: "3px",
              ...
              <div className="workbench-card">
                ...
                <p className="card-desc">
                  Manually configure REST endpoint fetches. Intercept latency,
                  headers, and status outcomes.
                </p>

                <div
                  className="api-config-grid"
                  style={{
                    display: "flex",
                    flexDirection: "column",
                    gap: "12px",
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      Simulated Network Latency ({apiLatency}ms)
                    </label>
                    <div
                      style={{
                        display: "flex",
              ...
              <div className="workbench-card">
                ...
                <p className="card-desc">
                  Mutate storage values in real time to verify reactive updates
                  in the OmniConsole UI.
                </p>

                <div
                  className="storage-tabs-bar"
                  style={{
                    display: "flex",
                    gap: "4px",
                    borderBottom: "1px solid rgba(255,255,255,0.05)",
                ...
                >
                  <button
                    ...
                    style={{
                      ...
                      border: "none",
                      color:
                        sandboxStorageTab === "cookies" ? "#a78bfa" : "#86868b",
                      fontSize: "11px",
                      cursor: "pointer",
                      padding: "4px 8px",
                      fontWeight: 600,
                  ...
                  <button
                    ...
                    style={{
                      ...
                      border: "none",
                      color:
                        sandboxStorageTab === "local" ? "#a78bfa" : "#86868b",
                      fontSize: "11px",
                      cursor: "pointer",
                      padding: "4px 8px",
                      fontWeight: 600,
                  ...
                  <button
                    ...
                    style={{
                      ...
                      border: "none",
                      color:
                        sandboxStorageTab === "session" ? "#a78bfa" : "#86868b",
                      fontSize: "11px",
                      cursor: "pointer",
                      padding: "4px 8px",
                      fontWeight: 600,
                ...
                <div
                  className="keychain-table-container"
                  style={{
                    maxHeight: "160px",
                    overflowY: "auto",
                    marginBottom: "12px",
                            ...
                            >
                              ...
                              <td style={{ padding: "6px" }}>
                                <button
                                  ...
                                  onClick={() =>
                                    handleSandboxDelete(cookie.key)
                                  }
                                >
                                  <svg
                                    width="11"
                            ...
                            >
                              ...
                              <td style={{ padding: "6px" }}>
                                <button
                                  ...
                                  onClick={() => handleSandboxDelete(item.key)}
                                >
                                  <svg
                                    width="11"
                                    height="11"
                ...
                <div
                  className="sandbox-storage-form"
              ...
              <div className="workbench-card workbench-card-alert">
                <div className="card-header-row">
                  <span className="card-number" style={{ color: "#f87171" }}>
                    04
                  </span>
                ...
                <p className="card-desc">
                  Simulate fatal uncaught runtime script exception boundaries
                  and async promise rejections.
                </p>

                <div
                  className="exception-btn-group"
            ...
            <div className="workbench-profiler-column">
              ...
              <div className="workbench-card">
                <h3
                  className="profiler-section-title"
                  style={{
                    fontSize: "14px",
                    fontWeight: 600,
                    color: "#fff",
                ...
                <p className="card-desc">
                  Monitor hardware stress simulation metrics in real-time as
                  logs flow.
                </p>
                    ...
                    >
                      <svg width="70" height="70" viewBox="0 0 100 100">
                        ...
                        <circle
                          ...
                          cy="50"
                          r="40"
                          stroke={cpuMetric > 80 ? "#f87171" : "#a78bfa"}
                          strokeWidth="8"
                          strokeDasharray="251.2"
                          strokeDashoffset={251.2 - (251.2 * cpuMetric) / 100}
                          fill="none"
                          strokeLinecap="round"
                          style={{
                    ...
                    >
                      <svg width="70" height="70" viewBox="0 0 100 100">
                        ...
                        <circle
                          ...
                          cy="50"
                          r="40"
                          stroke={memoryMetric > 300 ? "#fbbf24" : "#34d399"}
                          strokeWidth="8"
                          strokeDasharray="251.2"
                          strokeDashoffset={
                            251.2 - (251.2 * Math.min(500, memoryMetric)) / 500
                          }
                          fill="none"
                          strokeLinecap="round"
                          style={{
                ...
                >
                  ...
                  >
                    ...
                    <span
                      style={{
                        color: fpsMetric < 40 ? "#f87171" : "#a78bfa",
                        fontWeight: "bold",
                        fontFamily: "var(--font-mono)",
                      }}
                    >
                  ...
                  <div
                    className="svg-sparkline-container"
                    style={{ height: "50px" }}
                  >
                    <svg
                      width="100%"
                ...
                <div
                  className="perf-benchmark-control-box"
                  style={{
              ...
              >
                ...
                <p className="card-desc">
                  Active trace list of events intercepted in the sandbox
                  workspace.
                </p>

                <div
                  className="workbench-events-list"
                  style={{
                    flex: 1,
                    maxHeight: "420px",
                    overflowY: "auto",
                ...
                >
                  {workbenchEvents.length === 0 ? (
                    ...
                    workbenchEvents.map((evt, idx) => (
                      <div
                        key={idx}
                        className={`event-feed-row ${evt.status}`}
                        style={{
                          display: "flex",
                          gap: "8px",
                          padding: "4px 6px",
                          borderRadius: "4px",
                          background: "rgba(255,255,255,0.01)",
                          borderLeft: `2px solid ${evt.status === "error" ? "#f87171" : evt.status === "warning" ? "#fbbf24" : evt.status === "success" ? "#34d399" : "#a78bfa"}`,
                        }}
                      >
